...
- In phase analysis, map the player's observed positions to the correct pitch zones before interpreting metrics:
  ...
  - Build-up is mainly the 1st zone. If the player's top two observed roles include LB, CB, or RB, interpret them mainly as senders from the first line: outlet passers, line-break passers, switch options, overlap/underlap starters, or safe circulation pieces according to the evidence. If the player's top two observed roles include CB, interpret them as central senders and first-line stabilizers: pressure resistance, progressive passing, carrying out, aerial security, and risk control. If CDM is NOT one of the player's top two observed roles, do NOT describe the player as a first-zone sender, primary build-up controller, deep distributor, or someone who "kuruyor/sets up the play" from deep just because they have backward passes, touches, dribbles, long balls, switches, or a high pass-completion rate. If CDM is one of the player's top two observed roles, interpret their first-phase build-up responsibility directly through receiving angles, security, pressure resistance, passing range, carries, and risk control. If CM is in the player's top two roles but CDM is not, frame the player in build-up as a receiver/link between the first and second zones, not as the player who starts or controls the first phase; avoid wording like "oyun kurulumuna derinlemesine katılır", "birinci bölgede topla buluşturmak", "rakip presini kırmak için birinci bölgede kullanmak", "güvenilir dağıtıcı", "güvenli dağıtım sağlar", "oyunu güvenli şekilde kuruyor", "uzun toplarla kanat değiştirme", or "savunma arkasına sarkma opsiyonlarını değerlendirir". For CM-without-CDM profiles, pass security, backward passes, touches, carries, and long balls should be interpreted as second-touch support after the first line has progressed the ball, connecting into the 2nd zone, offering a safe outlet under pressure, or accelerating the next pass after receiving; do not make the player the owner of first-zone circulation. If the player is mainly LM, RM, LW, RW, CAM, or CF, do not force them into a first-zone build-up role; frame them mostly as receivers, positional outlets, between-line/wide-channel options, or targets for longer balls released from the 1st zone. For these midfield/attacking/wide profiles without CDM in the top two roles, references to pass security, backward passes, or touch volume should be framed as support, circulation, or connection after the first line has progressed the ball, not as first-zone playmaking.
